...
trait GenericConfigParser extends RecipeParser {
  ...
  def cfgStatement = cfgStatementNum | cfgStatementStr | cfgStatementSub

  // A bit of type acrobatics because this is recursive (via cfgStatementSub), the tuple is translated into the proper class in either cfgStatementSub or cfg
  def cfgBlock: Parser[(String, List[CfgStatement])] = "cfg" ~> ident ~ ("{" ~> cfgStatement.* <~ "}") ^^ { case id ~ st => (id, st) }
