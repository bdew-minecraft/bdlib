...
trait BaseRotateableBlock extends Block {
  ...
  override def onBlockPlacedBy(world: World, x: Int, y: Int, z: Int, ent: EntityLivingBase, stack: ItemStack) {
    val validRotations = getValidFacings
    var dir = getDefaultFacing
    if (ent != null) {
      val pitch: Int = Math.round(ent.rotationPitch)
      val yaw: Int = MathHelper.floor_double(ent.rotationYaw * 4.0F / 360.0F + 0.5D) & 3
      dir = yaw match {
